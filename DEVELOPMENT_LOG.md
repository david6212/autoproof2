...
## הפספורט — "הרכב שלי" (אוגוסט 2026)

הפיצ'ר שהופך את האפליקציה ממקום שקונים בו פעם ל-4 שנים למקום שנשארים בו.
קונה רוכש רכב → הרכב נכנס לתיק פרטי → הבעלים מתעד טיפולים לאורך שנים →
לוחץ למכור → ההיסטוריה כבר מחוברת → הקונה הבא רואה רכב מתועד → המעגל חוזר.

**שני יומנים נפרדים, וזו ההחלטה המרכזית:**
- `services` — טיפולים. append-only, נאכף בכללי האבטחה (`allow update: if false`)
  וגם בריפוזיטורי שלא חושף מתודה כזו. תיקון = רשומה חדשה; שתיהן נשארות.
- `expenses` — תדלוק, ניקיון, חניה. ניתן לעריכה, ולעולם לא נחשף לקונה.

**מסמכים** פרטיים כברירת מחדל, מתג שיתוף לכל אחד. רישיון רכב/ביטוח/חוזה
מציגים אזהרה לפני שיתוף — הם נושאים ת"ז וכתובת.

**העברת בעלות** בקוד מסירה בן 6 תווים. הקוד הוא מזהה המסמך ו-`list` חסום,
כך שאי אפשר לגלות קוד בלי לקבל אותו. **המוכר עושה את כל הכתיבה** — הקונה
לא רשאי לגעת במודעה של אחר, ובתוך batch סירוב אחד מפיל את הכל.

```
ce0ebd6 16/08  פספורט רכב: התיק, הארנק וארון המסמכים
cb29415 16/08  סגירת הלולאה: פרסום בלחיצה מהפספורט
f162979 16/08  העברת בעלות: המעגל נסגר
23670c7 16/08  תזכורות, קריאות שירות, ומודיעין שוק
bb0f9cb 16/08  השמורים חוזרים לכותרת הבית, וההשוואה איתם
ad9f8c9 16/08  רכבים שהיו בבעלותי
```
